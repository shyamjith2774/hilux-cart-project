...
const Navbar = () => {
  ...
  const { search, setsearch, Tokens, settokens } = useContext(ShopContext)
  ...
  return (
    <div className='flex items-center justify-between py-3 md:py-5   '>
      ...
      <Link to='/'>
        <img src={assets.logo1} alt="logo1" className='w-36 h-10  md:scale-130 transition  ease-in-out ' />
      </Link>
      ...
      <div className='flex items-center gap-5'>
        ...
        <div className='group relative'>
          ...
          <img onClick={() => Tokens ? null : navigate('/login')} src={assets.profile_icon} className='flex w-5' alt="" />
          ...
          {Tokens &&
            <div className='hidden xl:hidden lg:group-hover:block  absolute right-0  '>
              <div onClick={logout} className='flex items-center gap-2 flex-col w-32 px-2 py-4 mt-4 bg-slate-200 text-gray-600 font-semibold rounded-3xl' >
                <NavLink to='/login' className='cursor-pointer  hover:text-black' >Logout</NavLink>
